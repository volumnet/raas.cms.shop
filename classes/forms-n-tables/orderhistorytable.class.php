...
class OrderHistoryTable extends \RAAS\Table
{
    ...
    public function __construct(array $params = array())
    {
        ...
        $columns['post_date'] = array(
            ...
            'callback' => function($row) use ($view) { return date($view->_('DATETIMEFORMAT'), strtotime($row->post_date)); }
        );
        $columns['status_id'] = array(
            'caption' => $this->view->_('ORDER_STATUS'),
            'callback' => function($row) use ($view) { return htmlspecialchars($row->status->id ? $row->status->name : $view->_('ORDER_STATUS_NEW')); }
        );
        $columns['paid'] = array(
        ...
        $columns['description'] = array(
            ...
            'callback' => function($row) use ($view) { return htmlspecialchars($row->description); }
        );
        $columns['uid'] = array(
            'caption' => $this->view->_('AUTHOR'),
            'callback' => function($row) use ($view) { return htmlspecialchars($row->user->full_name ?: $row->user->login); }
        );
        $defaultParams = array(
            'columns' => $columns,
