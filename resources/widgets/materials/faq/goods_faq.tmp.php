...
if ($Set) { ?>
    <div class="goods-faq goods-faq__list goods-faq-list" data-v-bind_block-id="<?php echo (int)$Block->id?>">
      <?php foreach ($Set as $comment) { ?>
          <div data-vue-role="goods-faq-item" class="goods-faq-list__item goods-faq-item" id="faq<?php echo (int)$comment->id?>" data-v-slot="vm">
            ...
            <?php if ($comment->answer) { ?>
                <div class="goods-faq-item__answer">
                  <?php if ($comment->answer_image->id) { ?>
                      <img class="goods-faq-item__image" loading="lazy" src="/<?php echo htmlspecialchars($comment->answer_image->tnURL)?>" alt="<?php echo htmlspecialchars($comment->answer_image->name ?: $comment->answer_name)?>" />
                  <?php } ?>
                  <div class="goods-faq-item__text goods-faq-item__text_answer">
                    ...
                    <div class="goods-faq-item__more" data-v-if="vm.overflowed">
                      <button type="button" class="btn btn-link" data-v-on_click="vm.toggle()" data-v-html="vm.active ? <?php echo htmlspecialchars(json_encode(HIDE))?> : <?php echo htmlspecialchars(json_encode(READ_ANSWER))?>">
