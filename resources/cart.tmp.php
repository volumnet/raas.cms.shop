      ...
      <table class="table table-striped" data-role="cart-table">
        <thead>
          <tr>
            ...
            <th class="span2"><?php echo AMOUNT?></th>
            <th class="span1"><?php echo SUM?></th>
            <th class="span1"></th>
          </tr>
        </thead>
        <tbody>
          <?php $sum = 0; foreach ($Cart->items as $row) { ?>
            <tr data-role="cart-item">
              <td><?php echo htmlspecialchars($row->name)?></td>
              <td><?php echo htmlspecialchars($row->meta)?></td>
              <td data-role="price" data-price="<?php echo number_format($row->realprice, 2, '.', '')?>" class="span1"><?php echo formatPrice($row->realprice)?></td>
              <td class="span2"><input type="text" class="span1" data-role="amount" name="amount[<?php echo htmlspecialchars((int)$row->id . '_' . $row->meta)?>]" value="<?php echo (int)$row->amount?>" /></td>
              <td data-role="sum" class="span1"><?php echo formatPrice($row->amount * $row->realprice)?></td>
              <td class="span1">
          ...
          <tr>
            <th colspan="4"><?php echo TOTAL_SUM?>:</th>
            <th data-role="total-sum"><?php echo formatPrice($sum)?>&nbsp;р.</th>
            <th></th>
          </tr>
        </tbody>
      </table>
      ...
      <article class="article">
        <div class="feedback">
          <div class="form-horizontal">
            <h3 class="form-title"><?php echo CHECKOUT?></h3>
            <?php if (!$success[(int)$Block->id]) { ?>
