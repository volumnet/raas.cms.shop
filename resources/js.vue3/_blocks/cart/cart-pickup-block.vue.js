...
export default {
    ...
    data() {
        let result = {
            ...
            listActive: true,
            popupPointId: null, // Открытая подсказка у точки с данным ID#
        };
        return result;
    },
    methods: {
        ...
        selectMapPoint(point) {
            ...
            this.$emit('update:modelValue', point.id);
            window.setTimeout(() => {
    ...
    watch: {
        methodsFilter() {
            let matchingPoints = this.filteredPoints.filter(x => x.id == this.modelValue);
        ...
        filteredPoints() {
            if (this.filteredPoints.length == 1) {
                this.$emit('update:modelValue', this.filteredPoints[0].id);
            }
        }
    }
}
