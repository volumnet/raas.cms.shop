...
export default {
  ...
  data() {
    ...
    return {
      ...
      instantUpdateFields: ["city", "cityURN", "delivery", "payment", "promo"], // Поля, обновление которых требует немедленного получения доп. данных
      ...
      window.setTimeout(() => {
        if (this.$refs.quickorder) {
          window.scrollTo({
            left: 0,
            top:
              $(this.$refs.quickorder).offset().top +
              this.$root.getScrollOffset(),
            behavior: "smooth",
          });
        }
      });
    ...
    adjustRightPane() {
      ...
      if (
        ...
        console.log(
          ...
          "floatInnerHeight: " + floatInnerHeight,
          "paneTop: " + paneTop,
